<script>
	$(function(){
		...
		wave1_news_list.my_tooltip();
	});//ready
	
var wave1_news_list = {

	confirm_delete_news:function(news_name,news_id){
		if(news_name && news_id){
			$('#delete_news_name_span').html(news_name);
			$('#news_id').val(news_id);
			$j('#delete_news_model').modal('show'); 
		}
	...
	delete_news:function(){
		...
		if(news_id){
			window.location='<?=base_url();?>admin/news/delete_news/'+news_id;
		}
	},
...
var cs_search = {
	...
	start_search:function(){
		...
		$.ajax({
		  type:'post',
		  url:'<?=base_url();?>admin/news/ajax/search_news',
...
<div class="row row-bottom-margin">
	<div class="col-md-12">
        ...
        <div class="sub-title">Search news here. You can sort the search results based on title, date and status.</div>
		<div style="clear: both"></div>


		<div class="grey-box">
        	<button class="btn btn-info" onclick="window.location='<?=base_url();?>admin/news/add'"><i class="fa fa-plus"></i> Add New News</button>
        </div>
		<div style="clear: both"></div>
		...
		<table class="case-study-list-table">
	    	<thead>
	    		<tr class="list-tr">
	    			<th class="list-case-title">News Title <i data-toggle="tooltip" title="Sort by title" class="fa fa-sort-alpha-asc cursor list-title-sort sort-icons my-tooltip" onclick="cs_search.sort_search('title');"></i></th>
...
<!--begin delete-->
<div id="delete_news_model" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i></button>
            <h3 id="myModalLabel">Delete News</h3>
            </div>
            <div class="modal-body">
                
                <p>
                    You are about to delete <strong><span id="delete_news_name_span"></span></strong>. Confirm delete?
                </p>
            ...
            <button class="btn btn-info" onclick="wave1_news_list.delete_news();"><i class="fa fa-trash-o"></i> Delete</button>
            </div>
        </div>
    </div>
</div>
...
<div id="record-per-page" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
        	<form method="post" action="<?=base_url();?>admin/news/update_records_per_page">
            <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i></button>
