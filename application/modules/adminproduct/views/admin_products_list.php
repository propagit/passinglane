...
<?php
	$search_keyword = ($this->session->userdata('keyword') ? $this->session->userdata('keyword') : '');
	$status = ($this->session->userdata('status') ? $this->session->userdata('status') : '');
	$sort = ($this->session->userdata('sort') ? $this->session->userdata('sort') : '');
	$category = ($this->session->userdata('category') ? $this->session->userdata('category') : '');
?>
	...
	<div class="col-md-12">
		...
		<form id="addProduct" method="post" action="<?=base_url()?>admin/product/search">
			...
			<div class="form-search-input">
				<input type="text" class="form-control input-text" id="name" name="keyword" value="<?=$search_keyword;?>"/>
			</div>
			...
			<div class="form-search-input">
				<select class="selectpicker" id="cat" name="sort">
	            	<option value="title" <?=$sort == 'title' ? 'selected="selected"' : '';?>>Name</option>	
	            	<option value="date" <?=$sort == 'date' ? 'selected="selected"' : '';?>>Recently Added</option>		
	            </select>
		...
		<table class="table table-hover">
	    	<thead>
	    		<tr class="list-tr" style="font-size: 12px" id="list-header-title">
                    ...
                    <th style="width: 6.25%; text-align: center; font-size: 12px !important">PREVIEW</th>
                    <th style="width: 12%; text-align: center; font-size: 12px !important">CROSS SALE</th>
	    			...
	    			<th style="width: 6.25%; text-align: center; font-size: 12px !important">ACTIVE</th>
	    	...
	    	<tbody>
	    		...
	    		<tr class="list-tr" id="product-<?=$product['id']?>">
                    ...
                    -->
                    <td style="width: 6.25%; ">
	    				<div class="all_tt center_icon" data-toggle="tooltip" title="Preview"><a target="_blank" href="<?=base_url();?>product_preview/<?=$product['id_title'];?>"><i class="fa fa-search blue-icon"></i></a></div>
	    			</td>
                     <td>
	    				<div class="all_tt center_icon" data-toggle="tooltip" title="Cross Sale"><i class="fa fa-paperclip blue-icon add-cross-sale" data-product-id=<?=$product['id'];?>></i></div>
	    			</td>
	    			<!--<td style="width: 6.25%; ">
...
<div id="ajax-cross-sale-modal"></div>

<script>

$j(function(){
	
	//load cross sale modal
	$j('.add-cross-sale').on('click',function(){
		get_cross_sale_modal($j(this).attr('data-product-id'));
	});
});//ready

function get_cross_sale_modal(product_id)
{
	$j.ajax({
		url: '<?=base_url()?>admin/product/ajax/load_cross_sell_modal',
		type: 'POST',
		data: ({product_id:product_id}),
		dataType: "html",
		success: function(html) {
			$j('#ajax-cross-sale-modal').html(html);
			$j('#cross-sale-modal').modal('show');
		}
	});
}
...
function update_stock(product_id)
{
	...
	$('#new_stock').val(stock);
	$j('#stockModal').modal('show');
}
function updatestock()
{
	$j('#stockModal').modal('hide');
...
function export_csv() {
	$j('#csvModal').modal('show');
}
function exportcsv() {
	$j('#csvModal').modal('hide');
...
<h3 id="myModalLabel" class="title-page">Product Categories</h3>
</div>
...
<div class="modal-header">
<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="fa fa-times"></i></button>
<h3 id="myModalLabel" class="title-page">Sale Price</h3>
</div>
<div class="modal-body">
